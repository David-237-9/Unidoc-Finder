...
// Time of the day to auto crawl
const CRAWLING_HOUR = Number(process.env.CRAWLING_HOUR || 4) // DEFAULT: 4 AM GMT
const CRAWLING_MINUTE = Number(process.env.CRAWLING_MINUTE || 0) // DEFAULT: 0 minutes

const MIN_HOURS_BETWEEN_CRAWLS = Number(process.env.MIN_HOURS_BETWEEN_CRAWLS || 4) // DEFAULT: 4 hours
...
/**
 * Returns the milliseconds until the next crawl time.
 * If the next crawl time is within 4 hours, it will wait until the next day to avoid crawling too frequently.
 * @returns {number} The milliseconds until the next crawl time.
 */
function timeUntilNextCrawl() {
    const now = new Date()

    const nextTime = new Date(now)
    nextTime.setUTCHours(CRAWLING_HOUR, CRAWLING_MINUTE, 0, 0)

    // If the next crawl time is in the past, schedule it for the next day
    if (nextTime <= now) nextTime.setUTCDate(nextTime.getUTCDate() + 1)

    const timeUntilCrawl = nextTime.getTime() - now.getTime()

    const minTimeBetweenCrawls = MIN_HOURS_BETWEEN_CRAWLS * 60 * 60 * 1000 // Convert hours to milliseconds
    if (timeUntilCrawl <= minTimeBetweenCrawls) // If the next crawl time is within the minimum hours between crawls, wait until the next day
        return timeUntilCrawl + 24 * 60 * 60 * 1000 // Add 24 hours to wait until the next day

    return timeUntilCrawl
}

/**
 * Resolves at the next crawl time
 */
function waitUntilNextCrawl() {
    return new Promise((resolve) => {
        const timeUntilCrawl = timeUntilNextCrawl()
        console.log(`Next crawl scheduled in ${Math.round(timeUntilCrawl / 1000 / 60)} minutes.`)
        setTimeout(resolve, timeUntilCrawl)
    });
}

/**
 * Runs the crawler immediately and then loops the crawler every day at the configured time.
 * @return {Promise<void>}
 */
async function run() {
    await crawler() // Run the crawler once immediately

    while (true) {
        console.log('Waiting until next crawling...')

        await waitUntilNextCrawl()

        console.log("It is time to crawl again! Starting the crawler...")

        await crawler() // Run the crawler again
    }
}

run() // Start the crawler loop
